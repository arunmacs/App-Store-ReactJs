...
class AppStore extends Component {
  state = {inputKeyword: '', activeTabId: 'SOCIAL'}

  searchInput = event => {
    this.setState({inputKeyword: event.target.value})
  }

  getSelectedCategoryAppsData = (id, inputValue) => {
    const activeTabApps = appsList.filter(eachApp => eachApp.category === id)
    const filteredApps = activeTabApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
    return filteredApps
  }
  ...
  render() {
    const {inputKeyword, activeTabId} = this.state
    const filterAppsList = this.getSelectedCategoryAppsData(
      activeTabId,
      inputKeyword,
    )
      ...
      <div className="app-store-container">
        ...
        <ul className="tabs-container">
          {tabsList.map(eachItem => (
            <TabItem
              tabDetails={eachItem}
              onClickUpdate={this.updateActiveTab}
